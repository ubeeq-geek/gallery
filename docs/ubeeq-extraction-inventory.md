...
Both private products now generate a revision-bound, checksummed deployment-artifact manifest after their CI builds. Their private infrastructure manifests pin those gated product revisions; publishing the artifacts to the product AWS accounts and deploying them remains the next operational hand-off.
...
Those jobs also install, test, and build the private product before evaluating the pinned Ubeeq contract, making the product compatibility result a release gate rather than a manifest-only check.
