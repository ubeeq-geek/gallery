...
export function CreatorLaunchChecklist({ creator }: { creator: StudioCreator }) {
  ...
  return <Card title={`Finish setting up ${creator.name}`} eyebrow={`Your ${brand.workspaceName} · ${completeCount}/4 core steps`} className="studio-launch-checklist">
    ...
    <ol className="studio-launch-steps">
      <li className={profileReady ? 'is-complete' : ''}><div><strong>Profile and branding</strong><span>{profileReady ? 'Profile details or branding have been added.' : 'Add a bio, links, avatar, or cover so visitors know whose Space this is.'}</span></div><Link className="auth-secondary-btn no-underline" to={`/studio/workspace?section=creators&creatorId=${encodeURIComponent(creator.creatorId)}&edit=${encodeURIComponent(creator.creatorId)}`}>{profileReady ? 'Edit profile' : 'Set up profile'}</Link></li>
