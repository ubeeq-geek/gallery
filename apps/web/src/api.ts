...
export const api = {
  ...
  async billingAccounts() { return handleJson(await fetch(`${API_BASE}/billing/accounts`, { headers: await authHeaders() })); },
  async billingPrices() { return handleJson(await fetch(`${API_BASE}/billing/prices`, { headers: await authHeaders() })); },
  async createBillingAccount(currency: 'USD' | 'EUR' | 'GBP' = 'USD') { return handleJson(await fetch(`${API_BASE}/billing/accounts`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(await authHeaders()) }, body: JSON.stringify({ currency }) })); },
  async billingOverview(accountId: string, period: string) { return handleJson(await fetch(`${API_BASE}/billing/accounts/${encodeURIComponent(accountId)}/overview?period=${encodeURIComponent(period)}`, { headers: await authHeaders() })); },
  async createBillingCheckout(accountId: string, priceId: string) { return handleJson(await fetch(`${API_BASE}/billing/accounts/${encodeURIComponent(accountId)}/checkout`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(await authHeaders()) }, body: JSON.stringify({ priceId, successUrl: `${window.location.origin}/billing?checkout=success`, cancelUrl: `${window.location.origin}/billing?checkout=canceled`, idempotencyKey: createIdempotencyKey() }) })); },
  async createBillingPortal(accountId: string) { return handleJson(await fetch(`${API_BASE}/billing/accounts/${encodeURIComponent(accountId)}/portal`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(await authHeaders()) }, body: JSON.stringify({ returnUrl: `${window.location.origin}/billing` }) })); },
  async downloadBillingStatement(accountId: string, period: string) { const response = await fetch(`${API_BASE}/billing/accounts/${encodeURIComponent(accountId)}/statements/${encodeURIComponent(period)}.csv`, { headers: await authHeaders() }); if (!response.ok) throw new Error('Could not download usage statement'); const url = URL.createObjectURL(await response.blob()); const link = document.createElement('a'); link.href = url; link.download = `usage-${period}.csv`; link.click(); URL.revokeObjectURL(url); },
  async operatorBillingQueue() { const [tasks, reconciliation] = await Promise.all([handleJson(await fetch(`${API_BASE}/support/billing-tasks`, { headers: await authHeaders() })), handleJson(await fetch(`${API_BASE}/billing/operator/queue`, { headers: await authHeaders() }))]); return { tasks, reconciliation }; },
  async decideBillingTask(taskId: string, payload: { outcome: string; reason: string; providerOperationId?: string; resultingAmountMinor?: number }) { return handleJson(await fetch(`${API_BASE}/support/billing-tasks/${encodeURIComponent(taskId)}/decision`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(await authHeaders()) }, body: JSON.stringify(payload) })); },
};
