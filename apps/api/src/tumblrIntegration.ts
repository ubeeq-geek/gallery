...
export interface TumblrConnector {
  ...
  creatorApplicationEncrypted?: { clientId?: string; clientSecret?: string; consumerKey?: string; consumerSecret?: string };
...
export const renderTumblrNpfV1 = (request: TumblrRenderRequest): TumblrNpfPost => {
  const selected = request.mode === 'announcement'
    ? []
    : request.assets.filter((asset) => !request.selectedAssetIds || request.selectedAssetIds.includes(asset.id));
...
const stateSignature = (payload: string, secret: string) => createHash('sha256').update(`${secret}\0${payload}`).digest('base64url');
...
export const verifyTumblrOAuthState = (value: string, secret: string, now = Date.now()): TumblrOAuthState => {
  ...
  const state = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as Partial<TumblrOAuthState>;
...
export class TumblrApiClient {
  ...
  private async response(response: Response): Promise<unknown> {
    ...
    if (!response.ok) throw new Error(`Tumblr API request failed (${response.status}).`);
