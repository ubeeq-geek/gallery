...
export class GhostIntegrationService {
  ...
  publicConnection(connection: GhostConnection) {
    const { encryptedKey: _key, webhookSecretHash: _webhookSecret, ...safe } = connection;
    return {
      ...safe,
      health: deriveIntegrationAccountHealth(nativeIntegrationHealthConnection({
        platform: 'ghost',
        connectionStatus: connection.state === 'ACTIVE' ? 'connected' : connection.state === 'ERROR' ? 'temporarily_unavailable' : 'disabled',
        lastSuccessfulSyncAt: connection.lastSyncAt,
        lastIssue: connection.lastError ? { code: 'sync_failed', message: connection.lastError, remediation: 'Review the Ghost connection and retry.' } : undefined
      }))
    };
  }
...
export const registerGhostRoutes = (
  ...
  requireAdmission: (input: { operation: 'connect' | 'import' | 'publish' | 'update_remote' | 'delete_remote' | 'reconcile'; creatorId: string; connectionId?: string; workId?: string }) => Promise<void> = async () => undefined
) => {
  ...
  app.post('/api/integrations/ghost/connections', requireAuth, async (req, res) => { try { if (!await canAccessCreator(req, res, req.body?.creatorId)) return; await requireAdmission({ operation: 'connect', creatorId: req.body.creatorId }); res.status(201).json(await service.connect(req.body, req.authUser!.userId, canCreateManagedConnection(req))); } catch (e) { fail(res, e); } });
  ...
  app.post('/api/integrations/ghost/connections/:id/sync', requireAuth, async (req, res) => { try { const c = service.getConnection(req.params.id); if (!c || !await canAccessCreator(req, res, c.creatorId)) return; await requireAdmission({ operation: 'import', creatorId: c.creatorId, connectionId: c.connectionId }); res.json(await service.sync(c.connectionId, req.authUser!.userId)); } catch (e) { fail(res, e); } });
  ...
  app.post('/api/ghost/publications/:id/reconcile', requireAuth, async (req, res) => { try { const p = service.getPublication(req.params.id); if (!p || !await canAccessCreator(req, res, p.creatorId)) return; await requireAdmission({ operation: 'reconcile', creatorId: p.creatorId, connectionId: p.connectionId, workId: p.workId }); res.json(await service.reconcile(p.publicationId, req.authUser!.userId)); } catch (e) { fail(res, e); } });
  ...
  app.post('/api/ghost/publications/:id/publish', requireAuth, async (req, res) => { try { const p = service.getPublication(req.params.id); if (!p || !await canAccessCreator(req, res, p.creatorId)) return; const eligibility = await requireEligibility(res, p.workId, p.connectionId); if (!eligibility) return; await requireAdmission({ operation: 'publish', creatorId: p.creatorId, connectionId: p.connectionId, workId: p.workId }); res.json(await service.publish(p.publicationId, req.authUser!.userId, req.body?.confirm === true)); } catch (e) { fail(res, e); } });
  app.post('/api/ghost/publications/:id/unpublish', requireAuth, async (req, res) => { try { const p = service.getPublication(req.params.id); if (!p || !await canAccessCreator(req, res, p.creatorId)) return; await requireAdmission({ operation: 'delete_remote', creatorId: p.creatorId, connectionId: p.connectionId, workId: p.workId }); res.json(await service.unpublish(p.publicationId, req.authUser!.userId, req.body?.confirm === true)); } catch (e) { fail(res, e); } });
  app.delete('/api/ghost/publications/:id', requireAuth, async (req, res) => { try { const p = service.getPublication(req.params.id); if (!p || !await canAccessCreator(req, res, p.creatorId)) return; await requireAdmission({ operation: 'delete_remote', creatorId: p.creatorId, connectionId: p.connectionId, workId: p.workId }); await service.remove(p.publicationId, req.authUser!.userId, req.body?.confirm === true); res.status(204).send(); } catch (e) { fail(res, e); } });
