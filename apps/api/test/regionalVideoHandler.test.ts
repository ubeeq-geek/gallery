...
describe('regional video handler', () => {
  ...
  it('validates, extracts, uploads, and persists paired frame jobs', async () => {
    ...
    const reserve = jest.fn().mockResolvedValue(undefined); const handler = createRegionalVideoHandler({ authorize: jest.fn(), ensurePlanJob: jest.fn().mockResolvedValue(true), tools: { probe: jest.fn().mockResolvedValue(probe), extractFrame }, download: jest.fn().mockResolvedValue('authoritative-hash'), uploadFrame, reserve, persistPlan, markUnavailable: jest.fn(), deferRetry: jest.fn() });
    ...
    expect(reserve).toHaveBeenCalledWith(job, expect.objectContaining({ mediaType: 'video', durationSeconds: 4 }), scanJobs[0].scanGroupId);
    expect(reserve.mock.invocationCallOrder[0]).toBeLessThan(persistPlan.mock.invocationCallOrder[0]);
  });
  ...
  it('releases reserved credits when exhausted plan persistence fails', async () => { const release = jest.fn().mockResolvedValue(undefined); const handler = createRegionalVideoHandler({ authorize: jest.fn(), ensurePlanJob: jest.fn().mockResolvedValue(true), tools: { probe: jest.fn().mockResolvedValue(probe), extractFrame: jest.fn().mockResolvedValue(undefined) }, download: jest.fn().mockResolvedValue('hash'), uploadFrame: jest.fn().mockResolvedValue(undefined), reserve: jest.fn().mockResolvedValue(undefined), release, persistPlan: jest.fn().mockRejectedValue(Object.assign(new Error('write'), { name: 'PersistenceFailure' })), markUnavailable: jest.fn(), deferRetry: jest.fn() }); await expect(handler(event(job, '5'))).resolves.toEqual({ batchItemFailures: [] }); expect(release).toHaveBeenCalledWith(job, expect.any(String), 'PersistenceFailure'); });
});
