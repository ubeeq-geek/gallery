...
describe('Bluesky announcement broker client', () => {
  it('signs an idempotent broker request without exposing OAuth credentials', async () => {
    ...
    await expect(sendBlueskyAnnouncement(config, 'did:plc:test', { text: 'Announcement' }, 'attempt-1', '2026-08-24T12:00:00.000Z')).resolves.toEqual({ uri: 'at://did:plc:test/app.bsky.feed.post/rkey', cid: 'cid-1' });
    ...
    expect(JSON.parse(body)).toMatchObject({ did: 'did:plc:test', post: { text: 'Announcement' }, idempotencyKey: 'attempt-1', createdAt: '2026-08-24T12:00:00.000Z' });
